...
export default class App extends Component {
    ...
    handleMovie = () => {
        ...
        this.fetchAPI(path);
        this.setState({ hasLoaded: true, useData: false });

    }
    handlePeople = () => {
        let path = "https://ghibliapi.herokuapp.com/people";
        this.fetchAPI(path);
        this.setState({ hasLoaded: true, useData: true })
    }
    render() {
        ...
        return (
            <Fragment>
                ...
                {!this.state.hasLoaded ? null : this.state.dataArr.map(
                    (data, idx) => {
                        let itm1 = this.state.useData ? data.name : data.title;
                        let itm2 = this.state.useData ? data.age : data.director;
                        let itm3 = this.state.useData ? data.gender : data.description;
                        ...
                        return <div key={idx} className="shadow-lg p-3 mb-5 bg-white rounded">
                            <div className="card">
                                ...
                                <div className="card-body">
                                    ...
                                    {!this.state.useData ? null : <a href={data.url} target="_blank" rel="noopener noreferrer" className="btn btn-primary">{`View ${data.name}`}</a>}
                                </div>
                            </div>
                        </div>
                    }
